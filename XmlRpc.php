...
class Widget_XmlRpc extends Widget_Abstract_Contents implements Widget_Interface_Do
{
     ...
     */
    public function checkAccess($name, $password, $level = 'contributor', $union = '')
    {
        /** 记录唯一标识 */
        Typecho_Cookie::set('__typecho_union', $union, 0);
        /** 鉴权 */
    ...
    public function NbGetUser($union, $userName, $password)
    {
        ...
        if (!$this->checkAccess($userName, $password, "administrator", $union)) {
            return $this->error;
        }
        ...
        $statArray = array(
            ...
            "comment" => array(
                ...
                "me" => $this->db->fetchObject($this->db->select(array('COUNT(coid)' => 'num'))
                    ->from('table.comments')
                    ->where('table.comments.authorId = ?', $this->uid))->num,
    ...
    public function NbGetPost($union, $userName, $password, $postId, $struct)
    {
        ...
        $select->where('table.contents.authorId = ?', $this->uid);
     ...
     * @access public
     */
    public function NbEditPage($union, $userName, $password, $content, $union)
    {
        $content['type'] = 'page';
        return $this->NbNewPost($union, $userName, $password, $content);
    }
        ...
        if ($struct['method'] == "insert") {
            ...
            try {
                $select = $this->db->select()->from('table.dynamics')
                    ->where('table.dynamics.authorId = ?', $this->uid);
